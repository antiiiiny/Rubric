...
See [stages.md](stages.md) — **Stages 0–6 are COMPLETED.** The monorepo scaffolding exists, the frontend/backend/ai-service health-check chain works end-to-end, the backend has a layered architecture with centralized error handling, Zod validation, structured (secret-redacted) logging, and a passing Vitest test suite — Postgres (via Docker Compose, host port 5433) plus JWT/bcrypt authentication with `users`/`courses`/`course_members` schema and RBAC middleware are live and tested — course management (CRUD + enrollment) is in place — the quiz system (`assessments`/`questions`/`rubric_criteria`/`submissions`/`answers`, faculty authoring UI, publish flow, student quiz-taking UI, deterministic MCQ auto-grading) is live end-to-end — and AI evaluation is now the full LangGraph multi-agent pipeline (`ai-service`: parallel Concept Evaluator/Accuracy Critic/Completeness Evaluator agents → Judge reconciliation → conditional conflict-resolution cycle → Feedback Agent, each a schema-validated Groq call), persisted with a full per-run audit trail (`evaluations`/`criterion_results` for the judge's final decision, `evaluation_runs`/`agent_results` for the multi-agent trace) and deterministic backend-side weighted scoring, surfaced to both faculty and students including a collapsible evaluator trace and generated feedback. Next up: Stage 7 (Assignment Evaluation / document uploads).
