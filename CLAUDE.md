...
See [stages.md](stages.md) — **Stages 0–10 are COMPLETED.** The monorepo scaffolding exists, the frontend/backend/ai-service health-check chain works end-to-end, the backend has a layered architecture with centralized error handling, Zod validation, structured (secret-redacted) logging, and a passing Vitest test suite — Postgres (via Docker Compose, host port 5433) plus JWT/bcrypt authentication with `users`/`courses`/`course_members` schema and RBAC middleware are live and tested — course management (CRUD + enrollment) is in place — the quiz system (`assessments`/`questions`/`rubric_criteria`/`submissions`/`answers`, faculty authoring UI, publish flow, student quiz-taking UI, deterministic MCQ auto-grading) is live end-to-end — AI evaluation is the full LangGraph multi-agent pipeline (`ai-service`: parallel Concept Evaluator/Accuracy Critic/Completeness Evaluator agents → Judge reconciliation → conditional conflict-resolution cycle → Feedback Agent, each a schema-validated Groq call), persisted with a full per-run audit trail (`evaluations`/`criterion_results` for the judge's final decision, `evaluation_runs`/`agent_results` for the multi-agent trace) and deterministic backend-side weighted scoring — assignment/document evaluation is live (PDF/DOCX upload via multer → safe text extraction via `pdf-parse`/`mammoth` → deterministic required-section detection → the same multi-agent pipeline) — and faculty review is live (`faculty_reviews`, stored separately from the AI's `evaluations`/`criterion_results` so neither ever overwrites the other; faculty can approve, override the final score directly, or override individual criterion statuses with the score deterministically recomputed from faculty-defined weights; students see the faculty-final `effectiveScore` once reviewed while the original AI evaluation stays independently visible) — and course-level concept-mastery analytics are live (a deterministic SQL aggregation over `criterion_results` grouped by concept name, surfaced as a faculty-only table with a common-misconception flag) — and a UI/UX accessibility pass (proper `<label>`s on every form control, keyboard-operable native elements throughout, responsive flex-wrap fixes, type-neutral wording for the shared quiz/assignment page) is done via code review (no browser-automation tool available in this environment, so not visually verified). Next up: Stage 11 (Testing & Reliability).
